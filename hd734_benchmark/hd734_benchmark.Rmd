...
read_vcf <- function(file_name) {
  ...
  .infosplit <- str_split_fixed(.vcf$info, regex("[=;]"), Inf)[,c(2, 4, 12, 14)]
  
  .vcf$coverage <- as.numeric(.infosplit[,1])
  .vcf$frequency <- as.numeric(.infosplit[,2])
  .vcf$nb.r <- as.numeric(.infosplit[,3])
  .vcf$nb.p <- as.numeric(.infosplit[,4])
  .vcf$info <- NULL
...
read_vcf_with_metadata <- function(file_name, primer_set, replica, ratio, type) {
  ...
  pval <- with(.vcf, 1 - pbinom(count, coverage, nb.p / (1-nb.p)/ coverage))#pnbinom(count, nb.r, nb.p))
  #pval <- p.adjust(pval)

  .vcf$qual <- ifelse(pval == 0, 1000, -10 * log10(pval))
